...
#define SCS_PLUGIN_MMF_NAME "Local\\SCSTelemetry"
...
typedef struct scsTelemetryMap_s
{
	...
	struct {
		float fuelCapacity;            // 704
		float fuelWarningFactor;       // 708
		float adblueCapacity;          // 712
		float adblueWarningFactor;     // 716
		float airPressureWarning;      // 720
		float airPressurEmergency;     // 724
		float oilPressureWarning;      // 728
		float waterTemperatureWarning; // 732
		float batteryVoltageWarning;   // 736
		float engineRpmMax;			   // 740
		float gearDifferential;        // 744
		float cargoMass;               // 748
		float truckWheelRadius[16];    // 752
		float gearRatiosForward[24];   // 816
		float gearRatiosReverse[8];    // 912
		float unitMass;                // 944
	}config_f;
	//----- END OF third ZONE AT OFFSET 944 -----//
	struct {
		float speed;         // 948
		float engineRpm;     // 952
		float userSteer;     // 956
		float userThrottle;  // 960
		float userBrake;     // 964
		float userClutch;    // 968
		float gameSteer;     // 972
		float gameThrottle;  // 976
		float gameBrake;
		float gameClutch;
	...
	struct {
		bool truckWheelSteerable[16]; // 1500
		bool truckWheelSimulated[16]; // 1516
		bool truckWheelPowered[16];   // 1532
		bool truckWheelLiftable[16];  // 1548

		bool isCargoLoaded; // 1564
		bool specialJob;    // 1565
	}config_b;

	struct {
		bool parkBrake;				  // 1566
		bool motorBrake;     		  // 1567
		bool airPressureWarning;  	  // 1568
		bool airPressureEmergency;	  // 1569
		bool fuelWarning;             // 1570
		bool adblueWarning;           // 1571
		bool oilPressureWarning;      // 1572
		bool waterTemperatureWarning; // 1573
		bool batteryVoltageWarning;
		bool electricEnabled;
		bool engineEnabled;
		bool wipers;              // 1577
		bool blinkerLeftActive;   // 1578
		bool blinkerRightActive;  // 1579
		bool blinkerLeftOn;       // 1580
		bool blinkerRightOn;	  // 1581
		bool lightsParking;       // 1582
		bool lightsBeamLow;       // 1583
		bool lightsBeamHigh;      // 1584
		bool lightsBeacon;        // 1585
		bool lightsBrake;         // 1586
		bool lightsReverse;       // 1587
		bool lightsHazard;        // 1588
